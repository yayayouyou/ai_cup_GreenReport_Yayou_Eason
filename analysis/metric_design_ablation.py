...
def main():
    ...
    ap.add_argument('--train', default='../data_set/vpesg4k_train_1000 V1.json')
    ...
    for name, path in [('AI CUP zh (train-1000)', args.train), ('AI CUP zh (val-1000)', args.val)]:
        checks.append(schema_check(json.load(open(path)), name))
    ...
    print(f"\nML-Promise overall: {tot_v} violations / {tot_n} items = {100*tot_v/tot_n:.1f}%"
          f"   AI CUP: 0 violations (schema strictly enforced)")
    ...
    for vname, (lab, wt) in variants.items():
        ...
        print(f"    {vname:42s} worst->best ΔS={dS:+.4f}   cascade part {casc:+.4f}"
