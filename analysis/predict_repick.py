...
def main():
    ...
    ap.add_argument('--repick',action='store_true',help='per-item argmax over real classes when an open gate meets an N/A head')
    ...
    def assemble(sid,calib,rules,stype,t,offsets,gate=None):
        ...
        raw={f'_raw_{f}':picks[f] for f in FIELDS}   # module outputs BEFORE the schema cascade
        ...
        if args.repick:
            ...
            if picks['promise_status']=='Yes' and picks['verification_timeline']=='N/A':
                picks['verification_timeline']=_rp('verification_timeline')
            if picks['evidence_status']=='Yes' and picks['evidence_quality']=='N/A':
                picks['evidence_quality']=_rp('evidence_quality')
        for _f in ('verification_timeline','evidence_quality'):
            raw[f'_probs_{_f}']=[float(x) for x in allprobs[_f]]
        picks.update(raw)
        return picks
